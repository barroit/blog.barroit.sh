...
from .directive import tag_init_ids_map, tag_get_ids_map, tag_directive
...
def merge_info(app, env, docs, other):
	dst = tag_get_ids_map(env)
	src = tag_get_ids_map(other)
...
def setup(app):
	app.connect('builder-inited', tag_init_ids_map)
	app.add_directive('tag', tag_directive)

	app.add_search_language(mixed_lang)
	app.add_builder(post_builder, override=True)
